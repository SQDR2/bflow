...
class InitTest(unittest.TestCase):
    def test_init_scaffolds_shared_assets_and_project_adapters(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((project_root / ".bflow" / "reports" / "README.md").exists())
            ...
            replay_prompt = (project_root / ".github" / "prompts" / "bflow-replay.prompt.md").read_text(encoding="utf-8")
            self.assertIn(".bflow/reports/<case-name>.latest.md", replay_prompt)
            self.assertIn("lifecycle.last_failure_report", replay_prompt)
            ...
            self.assertIn(".bflow/reports/<case-name>.latest.md", diagnose_prompt)
            ...
            self.assertIn("last_failure_report: null", case_template)
            ...
            reports_readme = (project_root / ".bflow" / "reports" / "README.md").read_text(encoding="utf-8")
            self.assertIn("Minimum report contents", reports_readme)
